...
class DecimalType extends TypeBase {
	...
	private void validateDecimal(String path, String string) {
		...
		try {
			...
			throw ItemscriptError.internalError(this,
					"validateString.value.could.not.be.parsed.into.double",
		...
		if (inArray != null) {
			...
			for (int i = 0; i < inArray.size(); ++i) {
				...
				try {
					...
					throw ItemscriptError.internalError(this,
						"validateString.inArrayString.could.not.be.parsed.into.double",
						pathValueParams(path, string));
				}
			...
			if (!matched) {
				throw ItemscriptError.internalError(this,
						"validateString.value.did.not.match.a.valid.choice",
						pathValueParams(path, string));
			}
		}
		if (notInArray != null) {
			...
			for (int i = 0; i < notInArray.size(); ++i) {
				...
				try {
					...
					throw ItemscriptError.internalError(this,
						"validateString.notInArrayString.could.not.be.parsed.into.double",
						pathValueParams(path, string));
				}
		...
		if (greaterThan != null) {;
			try {
				...
				throw ItemscriptError
						.internalError(
								this,
								"validateString.greaterThan.could.not.be.parsed.into.double",
								pathValueParams(path, string));
			}
			if (decValue <= greaterThanValue) {
				throw ItemscriptError.internalError(this,
						"validateNumber.value.is.less.than.or.equal.to.min",
						pathValueParams(path, string));
			}
		}
		if (lessThan != null) {
			try {
				...
				throw ItemscriptError
						.internalError(
								this,
								"validateString.lessThan.could.not.be.parsed.into.double",
								pathValueParams(path, string));
			}
			if (decValue >= lessThanValue) {
				throw ItemscriptError.internalError(this,
						"validateNumber.value.is.greater.than.or.equal.to.max",
						pathValueParams(path, string));
			}
		}
		if (greaterThanOrEqualTo != null) {
			try {
				...
				throw ItemscriptError
						.internalError(
								this,
								"validateString.greaterThanOrEqualTo.could.not.be.parsed.into.double",
								pathValueParams(path, string));
			}
			if (decValue < greaterThanOrEqualToValue) {
				throw ItemscriptError.internalError(this,
						"validateNumber.value.is.less.than.min",
						pathValueParams(path, string));
			}
		}
		if (lessThanOrEqualTo != null) {
			try {
				...
				throw ItemscriptError
						.internalError(
								this,
								"validateString.lessThanOrEqualTo.could.not.be.parsed.into.double",
								pathValueParams(path, string));
			}
			if (decValue > lessThanOrEqualToValue) {
				throw ItemscriptError.internalError(this,
						"validateNumber.value.is.greater.than.max",
						pathValueParams(path, string));
			}
		}
		if (equalTo != null) {
			try {
				...
				throw ItemscriptError
						.internalError(
								this,
								"validateString.equalTo.could.not.be.parsed.into.double",
								pathValueParams(path, string));
			}
			if (decValue != equalToValue) {
				throw ItemscriptError.internalError(this,
						"validateNumber.value.is.not.equal.to.equal.to",
						pathValueParams(path, string));
			}
		}
		/**
	 ...
	 * digit. If it finds a character that is not a Digit it signals the
	 * badDigit flag and breaks out of the loop.
	 * 
	 * @param string
